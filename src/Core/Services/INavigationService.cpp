...
ITurnBehavior::TurnStatus NavigationService::update()
{
	...
	if (Entity* entity = _owner.lock())
	{
		if (_moveBehavior != nullptr)
		{
			...
			{
				worldCtx->getEventManager().emit(sw::io::MarchStarted{
					_owner.getId(),
					static_cast<uint32_t>(currentPos.x),
					static_cast<uint32_t>(currentPos.y),
					static_cast<uint32_t>(targetPos.x),
					static_cast<uint32_t>(targetPos.y)});
			}
			...
			{
				worldCtx->getEventManager().emit(sw::io::UnitMoved{
					_owner.getId(), static_cast<uint32_t>(result.to.x), static_cast<uint32_t>(result.to.y)});
				if (result.targetReached || _currentTask->isReached(result.to))
				{
					worldCtx->getEventManager().emit(sw::io::MarchEnded{
						_owner.getId(), static_cast<uint32_t>(result.to.x), static_cast<uint32_t>(result.to.y)});
					_currentTask.reset();
				}
